...
export default function PaymentPage() {
  ...
  const loadOrders = () => {
    ...
    const paymentOrders = allOrders.filter(order => order.status === "payment" || order.status === "completed")
    paymentOrders.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())
    setOrders(paymentOrders)
    setIsLoading(false)
  }

  const getDepositApplied = (order: SalesOrder) => {
    const pi = order.paymentInfo
    if (!pi?.depositReceivedAt) return 0
    const dep = typeof pi.depositAmount === "number" && Number.isFinite(pi.depositAmount) ? pi.depositAmount : 0
    return Math.max(0, dep)
  }

  const getBalanceDue = (order: SalesOrder) => {
    const total = Number.isFinite(order.total) ? order.total : 0
    return Math.max(0, total - getDepositApplied(order))
  }

  const markFinalPaymentReceived = () => {
    if (!selectedOrder) return
    const nowIso = new Date().toISOString()
    updateOrderByNumber(selectedOrder.orderNumber, (order) => ({
      ...order,
      paymentInfo: {
        ...(order.paymentInfo ?? { depositAmount: 0 }),
        finalPaymentReceivedAt: nowIso,
      },
      updatedAt: nowIso,
    }))
    loadOrders()
    const refreshed = getAllOrders().find((o) => o.orderNumber === selectedOrder.orderNumber) ?? null
    setSelectedOrder(refreshed)
    showAlert("Final payment marked as received.", { title: "Payment received", actionText: "OK" })
  }
  ...
  const handleDeleteOrder = (orderNumber: string) => {
    if (!confirm(`Delete payment record ${orderNumber}? This cannot be undone.`)) return
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{filteredOrders.length} payment records</p>
          </div>
        </div>
        ...
        {paginatedOrders.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-lg border border-border bg-card py-16">
            <CheckCircle className="mb-4 h-12 w-12 text-muted-foreground" />
              <h3 className="mb-2 text-lg font-semibold text-foreground">No Payment Orders</h3>
            <p className="text-sm text-muted-foreground">
              Orders will appear here once invoice is completed
            </p>
          </div>
        ) : (
          <>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border bg-muted/50">
                      ...
                      <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-foreground">Payment</th>
          ...
          <div className="rounded-lg border border-border bg-card p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Total: RM {Number.isFinite(selectedOrder.total) ? selectedOrder.total.toFixed(2) : "0.00"}</p>
                <p className="text-sm text-muted-foreground">
                  Deposit: RM {(typeof selectedOrder.paymentInfo?.depositAmount === "number" && Number.isFinite(selectedOrder.paymentInfo.depositAmount) ? selectedOrder.paymentInfo.depositAmount : 0).toFixed(2)}{" "}
                  <span className="text-xs text-muted-foreground">({selectedOrder.paymentInfo?.depositReceivedAt ? "received" : "not received"})</span>
                </p>
                <p className="text-sm text-muted-foreground">Balance due: RM {getBalanceDue(selectedOrder).toFixed(2)}</p>
                <div className="flex items-center justify-between gap-2 mt-1">
                  ...
                  <Button
                    variant="outline"
                    className="bg-transparent"
                    onClick={markFinalPaymentReceived}
                    disabled={Boolean(selectedOrder.paymentInfo?.finalPaymentReceivedAt)}
                  >
                    {selectedOrder.paymentInfo?.finalPaymentReceivedAt ? "Received" : "Mark received"}
                  </Button>
                </div>
              </div>
            </div>
