...
import { getOfficialQuotationById } from "@/lib/official-quotation-storage"
...
function parseBudgetAverage(fromRaw: string, toRaw: string): number {
  const from = parseFloat(fromRaw)
  const to = parseFloat(toRaw)
  if (!Number.isFinite(from) && !Number.isFinite(to)) return 0
  if (Number.isFinite(from) && Number.isFinite(to)) return Math.round(((from + to) / 2) * 100) / 100
...
function SalesOrderContent() {
    ...
    const officialQuotationId = searchParams.get("official")
  ...
  // Prefill from Official Quotation (web form submission)
  useEffect(() => {
    if (!officialQuotationId) return
    if (editOrderNumber || previewOrderNumber) return

    const official = getOfficialQuotationById(officialQuotationId)
    if (!official) return

    const req = official.request

    setOrderMeta((prev) => ({
      ...prev,
      orderNumber: prev.orderNumber || official.id,
      orderDate: prev.orderDate || getCurrentDate(),
      orderTime: prev.orderTime || getCurrentTime(),
      madeBy: prev.madeBy || official.createdBy || prev.madeBy,
      isAutoGenerated: false,
    }))
    ...
    setEventData((prev) =>
      normalizeEventDates({
        ...prev,
        eventName: req.event.eventName || prev.eventName,
        eventDate: req.event.eventDate || prev.eventDate,
        eventType: req.event.eventType || prev.eventType,
        estimatedGuests: req.event.estimatedGuests || prev.estimatedGuests,
        customerPreferredSetupDate: req.event.takeOutSetupDate || prev.customerPreferredSetupDate,
        customerPreferredDismantleDate: req.event.takeOutDismantleDate || prev.customerPreferredDismantleDate,
        dismantleRequired: Boolean(req.event.returningRequired),
        budgetPerHead: parseBudgetAverage(req.event.budgetPerPersonFromRm, req.event.budgetPerPersonToRm),
        areaType: req.event.otherVenueType === "outdoor" ? "outdoor" : "indoor",
        locationAreaType: req.event.eventLocation === "others" ? "other" : "etre-cafe",
        locationAreaOther: req.event.otherAreaName || prev.locationAreaOther || "",
        areaSelection: req.event.eventLocation || prev.areaSelection || "",
      })
    )

    setCustomerData((prev) => ({
      ...prev,
      customerName: req.customer.name || prev.customerName,
      companyName: req.customer.companyName || prev.companyName,
      phone: req.customer.phone || prev.phone,
      email: req.customer.email || prev.email,
      billingAddress: req.customer.address || prev.billingAddress,
      deliveryAddress: req.customer.address || prev.deliveryAddress,
      specialRequest: req.customer.notes || prev.specialRequest,
    }))

    setQuotationOptions((prev) => ({
      ...prev,
      brandingRequirement: {
        type: req.branding.includeBrandLogo ? "logo" : req.branding.matchBrandColours ? "colour" : "none",
        logoOn: {
          dessert: !!req.branding.logoOnDessert,
          packaging: !!req.branding.logoOnPackaging,
          other: req.branding.logoOnOthers ? req.branding.logoOnOthersText : "",
        },
        colourOn: {
          dessert: !!req.branding.colourOnDessert,
          packaging: !!req.branding.colourOnPackaging,
          other: req.branding.colourOnOthers ? req.branding.colourOnOthersText : "",
        },
      },
      preferredMenuSelection: {
        ...prev.preferredMenuSelection,
        step1:
          req.menu.customisationLevel === "current" ||
          req.menu.customisationLevel === "partial" ||
          req.menu.customisationLevel === "full"
            ? req.menu.customisationLevel
            : prev.preferredMenuSelection.step1,
        notes: req.menu.customisationNotes || prev.preferredMenuSelection.notes,
        size: req.menu.dessertSize === "normal" || req.menu.dessertSize === "mini"
          ? req.menu.dessertSize
          : prev.preferredMenuSelection.size,
        drinks: {
          coffee: req.menu.drinks.includes("coffee"),
          tea: req.menu.drinks.includes("tea"),
          fizzy: req.menu.drinks.includes("fizzy"),
          other: req.menu.drinks.includes("others") ? req.menu.drinksOtherText : "",
        },
        packagingBox:
          req.menu.packaging === "customer-own" ||
          req.menu.packaging === "etre-existing" ||
          req.menu.packaging === "premium"
            ? req.menu.packaging
            : prev.preferredMenuSelection.packagingBox,
      },
    }))
  }, [officialQuotationId, editOrderNumber, previewOrderNumber])
      ...
      <div className="mt-6 flex gap-3">
            {!isFormEditable ? (
              ...
              <>
                ...
                >
                  <FileText className="h-4 w-4" />
                  {officialQuotationId ? "Generate official quotation" : "Generate Sales Order"}
                </Button>
              </>
            )}
          </div>
